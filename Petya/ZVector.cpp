...
BOOL CZVector::SetInfected(iterator *pFindInfo, WCHAR* pszData)
{
	WCHAR szBuf[16];
	wcscpy(szBuf, pszData);
	return SetInfected(pFindInfo, (WCHAR**)&szBuf);
}
...
BOOL CZVector::SetInfected(iterator *pFindInfo, WCHAR **pszData)
{
	...
	pRet->m_dwMark = 1;//∏–»æ±Í÷æ
	LeaveCriticalSection(&m_pcsLock);
	return bRet;
}

void CZVector::SetCollectComplete()
{
	EnterCriticalSection(&m_pcsLock);
	InterlockedExchange(&m_bCollectComplete, 1);
	LeaveCriticalSection(&m_pcsLock);
}
...
BOOL CZVector::GetString(iterator *it, WCHAR* pszOut)
{
	BOOL bRet = FALSE;
	StringInfo *pRet = NULL;
	if (FALSE == GetString(it, &pRet))
	{
		wcscpy(pszOut, pRet->m_pAryStrs[0]);
		if (NULL != pszOut)
		{
